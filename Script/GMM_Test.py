...
for i in range(len(test_files)):
 ...
 for j in range(len(GMMSPK)):
  gmm_name = "Speaker_"+str(j)
  loaded_gmm = mixture.GaussianMixture(n_components = cl_sz, covariance_type='diag')
  means = np.load("/home/pi/speaker_recognition/SR/GMM_Models/"+gmm_name + '_means.npy')
  covar = np.load("/home/pi/speaker_recognition/SR/GMM_Models/"+gmm_name + '_covariances.npy')
  loaded_gmm.weights_ = np.load("/home/pi/speaker_recognition/SR/GMM_Models/"+gmm_name + '_weights.npy')
  loaded_gmm.precisions_cholesky_ = np.load("/home/pi/speaker_recognition/SR/GMM_Models/"+gmm_name + '_precisions_cholesky.npy')
